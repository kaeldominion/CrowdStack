 ...
 * Get all tables with their availability status for a specific event (admin access)
 */
export async function GET(
  ...
  try {
    ...
    if (tablesError) {
      console.error("[Admin Event Tables API] Error fetching tables:", tablesError);
      throw tablesError;
    }
    ...
    if (availError) {
      console.error("[Admin Event Tables API] Error fetching availability:", availError);
      throw availError;
    }
    ...
    console.error("[Admin Event Tables API] Error:", error);
    return NextResponse.json(
...
/**
 * PUT /api/admin/events/[eventId]/tables
 * Bulk update table availability for an event (admin access)
 */
export async function PUT(
  ...
  try {
    ...
    // Verify the event exists and get its venue
    const { data: event, error: eventError } = await serviceSupabase
      .from("events")
      .select("id, venue_id")
      .eq("id", eventId)
    ...
    if (tablesError) {
      console.error("[Admin Event Tables API] Error verifying tables:", tablesError);
      throw tablesError;
    }
    ...
    if (upsertError) {
      console.error("[Admin Event Tables API] Error upserting availability:", upsertError);
      throw upsertError;
    }
