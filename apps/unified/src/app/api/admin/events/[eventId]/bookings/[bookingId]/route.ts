...
/**
 * PATCH /api/admin/events/[eventId]/bookings/[bookingId]
 * Update a table booking (admin access)
 */
export async function PATCH(
  ...
  try {
    ...
    const oldTableName = currentBooking.table?.name;
    ...
    if (body.status !== undefined && body.status !== currentBooking.status) {
      ...
      if (body.status === "cancelled" || body.status === "no_show") {
        ...
        updateData.cancellation_type = body.status === "no_show" ? "no_show" : "admin";
    ...
    if (updateError) {
      console.error("[Admin] Error updating booking:", updateError);
    ...
    try {
      ...
      console.error("[Admin] Failed to send booking notification email:", emailError);
    }
    ...
    console.error("[Admin] Error in booking PATCH:", error);
    return NextResponse.json(
...
/**
 * GET /api/admin/events/[eventId]/bookings/[bookingId]
 * Get a single booking (admin access)
 */
export async function GET(
  ...
  try {
    ...
    console.error("[Admin] Error in booking GET:", error);
    return NextResponse.json(
