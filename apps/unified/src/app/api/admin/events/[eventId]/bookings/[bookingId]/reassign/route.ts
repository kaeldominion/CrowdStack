...
/**
 * POST /api/admin/events/[eventId]/bookings/[bookingId]/reassign
 * Reassign a table from a no-show booking to a new guest (admin access)
 */
export async function POST(
  ...
  try {
    ...
    if (originalBooking.status !== "no_show") {
      const { error: updateOriginalError } = await serviceSupabase
        ...
        .update({
          ...
          cancellation_reason: "Table reassigned to another guest (admin)",
          updated_at: new Date().toISOString(),
        })
        .eq("id", bookingId);

      if (updateOriginalError) {
        console.error("[Admin] Error marking original booking as no_show:", updateOriginalError);
      ...
      try {
        ...
        console.error("[Admin] Failed to send no-show email:", emailError);
      }
    }
    ...
    if (createError) {
      console.error("[Admin] Error creating new booking:", createError);
    ...
    console.error("[Admin] Error in booking reassign:", error);
    return NextResponse.json(
