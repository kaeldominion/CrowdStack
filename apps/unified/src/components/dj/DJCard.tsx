...
export function DJCard({ 
  ...
  if (layout === "row") {
    return (
      <Link href={`/dj/${dj.handle}`} className="block group">
        <Card className="flex gap-4 p-3 hover:border-accent-primary/50 transition-all">
          {/* Avatar */}
          <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-xl overflow-hidden flex-shrink-0 bg-raised">
            {imageUrl ? (
              <Image
                src={imageUrl}
                alt={dj.name}
          ...
          <div className="flex-1 min-w-0 flex flex-col justify-center">
            <h3 className="font-bold text-primary group-hover:text-accent-primary transition-colors truncate">
              {dj.name}
            </h3>
      ...
      <Card className="overflow-hidden hover:border-accent-primary/50 transition-all" padding="none">
        ...
        <div className="relative aspect-square bg-raised">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4">
            <h3 className="font-bold text-lg text-white group-hover:text-accent-primary transition-colors">
              {dj.name}
            </h3>
            {showLocation && dj.location && (
...
export function DJCardSkeleton({ layout = "portrait" }: { layout?: "portrait" | "row" | "compact" }) {
  ...
  if (layout === "row") {
    return (
      <div className="flex gap-4 p-3 rounded-xl border border-border-subtle bg-glass animate-pulse">
        <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-raised flex-shrink-0" />
